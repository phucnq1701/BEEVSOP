  ...
  return (
    <View key={index} style={newLocal_8}>
      <View style={{ flexDirection: "row", alignItems: "center" }}>
        <FastImage source={require("./assets/Group2943.png")} style={newLocal_7} />
        <View style={{ padding: spacing[4] }}>
          <Text style={newLocal_6}>{item.TenGD}</Text>
          <Text style={newLocal_9}>{item.NgayNhap}</Text>
        </View>
      </View>
      <Text style={newLocal_10}>+{item.Diem} điểm</Text>
    </View>
  )
}
export const PayinHistoryScreen = observer(function PayinHistoryScreen() {
  ...
  return (
    <Screen style={ROOT} backgroundColor={color.palette.beehome} preset="fixed">
      <Header headerText="Lịch sử nạp điểm" leftIcon="back" onLeftPress={navigation.goBack} />
